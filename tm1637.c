...
static void TranslateBuffer(uint8_t DisplayNo, uint8_t* Buffer)
{
	...
	{
		if ((Displays[DisplayNo].STR_BUF[i] == '.') || (Displays[DisplayNo].STR_BUF[i] == ','))
		{
		...
		{
			CommaFlag = 0;
			if (i > 0) Displays[DisplayNo].STR_BUF[i-1] |= 0x80;
		}
	}
}
...
void TM1638_printf(uint8_t DisplayNo, TM1638_Alignment Alignment, const char* format, ...)
{
	...
	memset(Displays[DisplayNo].STR_BUF, 0x00, sizeof(Displays[DisplayNo].STR_BUF));
	...
	if (daRight == Alignment)
	{
		...
		{
			if ((0 != Displays[DisplayNo].STR_BUF[i]) || (PosShift != SEG_CONTROL_MAX-1))
			{
				Displays[DisplayNo].STR_BUF[PosShift] = Displays[DisplayNo].STR_BUF[i];
				Displays[DisplayNo].STR_BUF[i] = 0x00;
				PosShift--;
			}
		}
    ...
    for (uint8_t i=0; i<SEG_CONTROL_MAX; i++)
    {
    	if (!WriteByte(DisplayNo, Displays[DisplayNo].STR_BUF[i])) break;
    }
    StopSequence(DisplayNo);
}
...
uint8_t TM1638_Init(uint8_t DisplayNo, GPIO_TypeDef* DIO_Port, uint8_t DIO_Pin, GPIO_TypeDef* CLK_Port, uint8_t CLK_Pin)
{
	...
	return TM1638_SetBrightness(DisplayNo, BRIGHTNESS_DEF) && TM1638_ClearDisplay(DisplayNo);
}
